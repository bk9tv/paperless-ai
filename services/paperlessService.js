    ...
    try {
      ...
      try {
        if (updates.created) {
          ...
          if (!isValid(dateObject)) {
            console.warn(`[WARN] Invalid date format: ${updates.created}, omitting created date from update`);
            delete updates.created;
            updateData = { ...updates };
          } else {
            const normalizedDate = format(dateObject, 'yyyy-MM-dd');
            const placeholderDates = new Set(['1900-01-01', '1970-01-01', '1990-01-01', '2000-01-01']);
            if (placeholderDates.has(normalizedDate)) {
              console.warn(`[WARN] Likely placeholder date ${normalizedDate}, omitting created date from update`);
              delete updates.created;
              updateData = { ...updates };
            } else {
              updateData = {
      ...
      if (error.response) {
        console.error('[ERROR] Paperless API status:', error.response.status);
        console.error('[ERROR] Paperless API response:', error.response.data);
      }
      throw error;
    }
  }
